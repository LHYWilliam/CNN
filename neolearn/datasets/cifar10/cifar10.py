...
def load():
    ...
    classes = [str(name, encoding='utf-8') for name in files['batches.meta'][b'label_names']]

    x_train = numpy.array([files[file][b'data'].reshape(-1, 3, 32, 32) / 255.0
                           for file in (f'data_batch_{i + 1}' for i in range(5))]).reshape(-1, 3, 32, 32)
    t_train = numpy.array([files[file][b'labels'] for file in (f'data_batch_{i + 1}' for i in range(5))]).reshape(-1)
